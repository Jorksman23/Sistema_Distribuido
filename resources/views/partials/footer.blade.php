<footer class="bg-[#020817] text-gray-300 mt-16">
    ...
    <div class="max-w-7xl mx-auto px-6 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
            ...
            {{-- TELÉFONOS --}}
            @if(!empty($footerData['telefonos']))
                @foreach($footerData['telefonos'] as $telefono)
                    @if(!empty($telefono))
                        @php
                            // Extraer solo dígitos (por si viene con enlace)
                            $numero = preg_replace('/\D/', '', $telefono);

                            // Formatear con espacios
                            $numeroFormateado = '+'.substr($numero, 0, 3).' '
                                                .substr($numero, 3, 3).' '
                                                .substr($numero, 6, 3).' '
                                                .substr($numero, 9);
                        @endphp
                        ...
                        <div class="flex items-center gap-2 text-gray-300 mb-2">
                            <svg class="w-5 h-5 text-sky-400"
                                ...
                                <path stroke-linecap="round"
                                    stroke-linejoin="round"
                                    stroke-width="2"
                                    d="M3 5a2 2 0 012-2h3l2 5-2 2a11 11 0 005 5l2-2 5 2v3a2 2 0 01-2 2h-3C9.163 20 4 14.837 4 8V5z"/>
                            </svg>

                            <a href="https://wa.me/{{ $numero }}?text=Me%20gustaria%20saber%20del%20servicio"
                            target="_blank"
